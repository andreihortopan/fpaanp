...
export class RomaniaMap extends Component {
  ...
  handleOnClick = e => {
    ReactTooltip.hide();
    let county = [{ value: e.properties.NAME_1, label: e.properties.NAME_1 }];
    selectCounty(county);
  };
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        >
          <ZoomableGroup center={[24.5, 45.75]} disablePanning>
            <Geographies geography="/romania-counties.json" disableOptimization>
              {(geographies, projection) =>
                geographies.map((geography, i) => {
                  var countyData = mapData.find(function(county) {
                    if (county != null) {
