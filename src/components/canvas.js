...
const computeCountyDataArray = dataArray => {
  ...
  countyDataArray.forEach(currentCounty => {
    if (currentCounty.ngoNum != 0)
      currentCounty.average = Math.round(
        currentCounty.sum / currentCounty.ngoNum
      );
  });
};
...
const computeBarChartData = (dataArray, countyDataArray) => {
  ...
  if (countyNum === 1) {
    ...
    dataArray.forEach(ngo => {
      var currentLegislation = barChartCountyLegislationArray.find(function(
        legislation
      ) {
        if (legislation != null) {
  ...
  render() {
    ...
    const routes = [
      ...
      },
      {
        ...
        data: [
          countyDataArray,
          computeBarChartData(this.state.dataArray, countyDataArray)
        ]
      },
      {
    ...
    return (
      <React.Fragment>
        <GraphMenu
          ...
          filteredData={this.state.dataArray}
        />
        <Switch>
